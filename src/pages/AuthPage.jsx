...
const DEMO_ACCOUNTS = [
  { email: 'yamada@demo.com', password: 'demo1234', name: '山田 花子', role: '一般ユーザー', icon: Leaf, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100' },
  { email: 'suzuki@demo.com', password: 'demo1234', name: '鈴木 一郎', role: '一般ユーザー', icon: User, color: 'text-amber-700', bg: 'bg-amber-50 border-amber-200 hover:bg-amber-100' },
]

const FEATURES = [
  { icon: MapPin, color: 'bg-emerald-600', title: '狩猟記録', desc: '猟場・獲物・気温などを詳細に記録。チームと自動共有。' },
  { icon: Users, color: 'bg-sky-600', title: 'チーム管理', desc: 'メンバーを招待してリアルタイムで記録を共有・閲覧。' },
  { icon: FileText, color: 'bg-amber-600', title: '免許・許可証管理', desc: '狩猟免許・銃所持許可証の有効期限を一元管理。' },
  { icon: Camera, color: 'bg-stone-600', title: 'AI射撃分析', desc: '標的写真をAIが自動解析。スコアとグルーピングを記録。' },
]
...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      {/* 左パネル：自然写真背景 + アプリ紹介 */}
      <div className="lg:flex-1 relative flex flex-col justify-center px-8 py-12 lg:px-16 lg:py-16 overflow-hidden">
        {/* 背景：自然写真 */}
        ...
        {/* オーバーレイ */}
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(20,40,15,0.88) 0%, rgba(30,50,20,0.82) 50%, rgba(15,30,10,0.90) 100%)' }} />
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg">
            {FEATURES.map(f => {
              const Icon = f.icon
              return (
                <div key={f.title} className="flex items-start gap-3 bg-white/8 backdrop-blur-sm rounded-xl px-4 py-3 border border-white/10">
                  <div className={`w-8 h-8 ${f.color} rounded-lg flex items-center justify-center shrink-0 mt-0.5`}>
                    <Icon size={15} className="text-white" />
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-white">{f.title}</div>
                    <div className="text-xs text-emerald-200/60 mt-0.5 leading-relaxed">{f.desc}</div>
                  </div>
                </div>
              )
            })}
          ...
          {isDemoMode && (
            <div className="mt-8 inline-flex items-center gap-2 text-xs bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-3 py-1.5 rounded-full w-fit backdrop-blur-sm">
              <Zap size={11} />
              デモモード動作中
            </div>
          )}
      ...
      {/* 右パネル：ログインフォーム */}
      <div className="lg:w-[420px] flex flex-col justify-center px-6 py-8 lg:px-10 lg:py-16 bg-stone-50">
          ...
          <div className="mb-5 bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
            <button
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-stone-800">デモアカウントで体験する</span>
              </div>
              <ChevronRight size={16} className={`text-stone-400 transition-transform ${showDemo ? 'rotate-90' : ''}`} />
            </button>
                  ...
                  return (
                    <button
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-stone-800">{acc.name}</div>
                        <div className="text-xs text-stone-500">{acc.role}</div>
                      </div>
                      {isLoading
            ...
            <div className="flex rounded-lg bg-stone-100 p-1 mb-6">
              {['login', 'signup'].map(m => (
                <button key={m} onClick={() => { setMode(m); setError('') }}
                  className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${mode === m ? 'bg-white shadow text-stone-800' : 'text-stone-500'}`}>
                  {m === 'login' ? 'ログイン' : '新規登録'}
                </button>
              ))}
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {mode === 'signup' && (
                <label className="block">
                  <span className="text-xs text-stone-500 font-medium">表示名</span>
                  <input type="text" required value={form.displayName} onChange={e => set('displayName', e.target.value)}
                    placeholder="山田 太郎"
                    className="mt-1 w-full border border-stone-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" />
                </label>
              )}
              <label className="block">
                <span className="text-xs text-stone-500 font-medium">メールアドレス</span>
                <input type="email" required value={form.email} onChange={e => set('email', e.target.value)}
                  placeholder="example@email.com"
                  className="mt-1 w-full border border-stone-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" />
              </label>
              <label className="block">
                <span className="text-xs text-stone-500 font-medium">パスワード</span>
              ...
              <button type="submit" disabled={loading || !!demoLoading}
                className="w-full py-2.5 rounded-lg font-semibold text-sm text-white disabled:opacity-50 transition-colors mt-2 shadow-sm"
                style={{ background: 'linear-gradient(135deg, #059669 0%, #047857 100%)' }}>
